"use client";

import * as React from "react";
import Iconify from "@/components/ui/iconify";
import { cn } from "@/lib/utils";
import type { AlbumSection } from "@/collections/schema";

type Props = {
  section: AlbumSection;
  owned: number;
  total: number;
  onClick: () => void;
  className?: string;
  rtl?: boolean;
};

export const GenericSectionTile = React.memo(function GenericSectionTile({
  section,
  owned,
  total,
  onClick,
  className,
  rtl,
}: Props) {
  const isComplete = owned === total && total > 0;
  const percent = total === 0 ? 0 : Math.round((owned / total) * 100);
  const primary = section.primaryColor ?? "#0f172a";
  const accent = section.accentColor ?? primary;
  const labelEn = section.title.en ?? section.title.he ?? "";
  const labelHe = section.title.he;
  const label = rtl ? (labelHe ?? labelEn) : labelEn;

  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={`${label} — ${owned}/${total}`}
      className={cn(
        "group relative flex flex-col items-center justify-between gap-2 overflow-hidden rounded-3xl border-2 p-3 transition-all duration-200",
        "shadow-sm hover:-translate-y-0.5 hover:shadow-lg active:scale-[0.98]",
        "touch-manipulation",
        isComplete && "ring-2 ring-emerald-500/60",
        className
      )}
      style={{
        background: `linear-gradient(155deg, ${primary}28 0%, ${accent}18 60%, transparent 100%)`,
        borderColor: isComplete ? "#10b981" : `${primary}55`,
      }}
    >
      <span
        className="absolute inset-x-0 top-0 h-1"
        style={{ background: `linear-gradient(90deg, ${primary}, ${accent})` }}
        aria-hidden
      />
      <span
        className={cn(
          "absolute end-2 top-2 rounded-full px-2 py-0.5 font-mono text-[10px] font-bold tabular-nums shadow-sm",
          isComplete
            ? "bg-emerald-500 text-white"
            : owned === 0
              ? "bg-foreground/10 text-foreground/70"
              : "bg-background/90 text-foreground"
        )}
      >
        {owned}/{total}
      </span>
      {isComplete && (
        <span
          className="absolute start-2 top-2 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm"
          aria-hidden
        >
          <Iconify icon="lucide:check" className="size-3" />
        </span>
      )}

      <div className="mt-3 flex flex-col items-center gap-1.5">
        {section.flag ? (
          <span
            className="flex h-14 w-14 items-center justify-center rounded-2xl text-3xl shadow-sm ring-2 ring-white/40 sm:h-16 sm:w-16"
            style={{ backgroundColor: primary }}
            aria-hidden
          >
            <span className="drop-shadow-sm">{section.flag}</span>
          </span>
        ) : (
          <span
            className="font-heading flex h-14 w-14 items-center justify-center rounded-2xl text-xl font-black text-white shadow-sm ring-2 ring-white/40 sm:h-16 sm:w-16"
            style={{ backgroundColor: primary }}
            aria-hidden
          >
            {section.badge ?? label.charAt(0).toUpperCase()}
          </span>
        )}
        <span
          className="font-heading line-clamp-2 text-center text-sm leading-tight font-extrabold"
          style={{ color: primary }}
        >
          {label}
        </span>
      </div>

      <div className="bg-foreground/10 mt-1 h-1.5 w-full overflow-hidden rounded-full">
        <div
          className="h-full rounded-full transition-all duration-500"
          style={{
            width: `${percent}%`,
            background: isComplete ? "#10b981" : `linear-gradient(90deg, ${primary}, ${accent})`,
          }}
        />
      </div>
    </button>
  );
});
