...
export function AlbumRouter({ album, collectors }: Props) {
  ...
  if (album.layout === "world-cup-grouped") {
    return <AlbumPage collectors={collectors} />;
  }
  if (album.layout === "world-cup-flat-grouped") {
    return <GroupedAlbumPage album={album} />;
  }
  if (album.layout === "flat-sections" || album.layout === "team-grid") {
    return <GenericAlbumPage album={album} />;
  }
...
function MetadataOnlyView({ album }: { album: Album }) {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 lg:px-10">
      <div className="mb-6">
      ...
      >
        ...
        {album.title.he && album.title.en && album.title.he !== album.title.en && (
